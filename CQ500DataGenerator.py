...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, ID_list, batch_size=14, dim=(224, 224), n_channels=3, shuffle=True): #n_classes=1, 
        'Initialization'
        self.data_dir = 'D:/PyProjects/CQ500_ConvOuch/'
        self.dim = dim
    ...
    def __data_generation(self, batch_list_IDs):
        ...
        for i, ID in enumerate(batch_list_IDs):
            ...
            # else:
            #     pass
            batch_samples[i,] = np.load(self.data_dir + 'Slices/' + "CQ500-CT-"+ID + '.npy',allow_pickle=True)

            # load label
            data_obj = np.load(self.data_dir + 'Labels/' +"CQ500-CT-"+ ID + '.npy',allow_pickle=True)
